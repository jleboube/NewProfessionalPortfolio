...
// Enable CORS for frontend access to public API
// Support both domain name and localhost for development
// Support multiple domains via comma-separated values
const allowedOrigins = [
  process.env.FRONTEND_URL,
  process.env.DOMAIN_NAME,
  process.env.ADMIN_DOMAIN_NAME, // Admin panel needs to access its own API
  // Support additional domains via comma-separated ADDITIONAL_DOMAINS env var
  ...(process.env.ADDITIONAL_DOMAINS ? process.env.ADDITIONAL_DOMAINS.split(',').map(d => d.trim()) : []),
  'http://localhost:3005',
  'http://localhost:5173', // Vite dev server
  'http://localhost:6905', // Admin panel local dev
...
// These endpoints are not protected by auth and serve data to the React frontend
// MUST be defined BEFORE authMiddleware is applied
...
app.get('/api/public/resume', async (req, res) => {
  try {
    const data = await readData();
...
app.get('/api/public/blog', async (req, res) => {
  try {
    const data = await readData();
    const posts = (data.blogPosts || []).filter(post => post.status === 'published');
    res.json({
      enabled: data.blogEnabled || false,
      posts: posts
    });
  } catch (error) {
...
// Get settings (theme, blog enabled, etc.)
app.get('/api/public/settings', async (req, res) => {
  try {
    const data = await readData();
    res.json({
      colorTheme: data.colorTheme || 'ocean',
...
app.listen(PORT, () => {
  console.log('==========================================');
  console.log('Portfolio Admin Panel Started');
  console.log('==========================================');
  console.log(`Internal Port: ${PORT}`);

  if (process.env.ADMIN_DOMAIN_NAME) {
    console.log(`Admin URL: ${process.env.ADMIN_DOMAIN_NAME}`);
    console.log(`Public API: ${process.env.ADMIN_DOMAIN_NAME}/api/public/*`);
  } else {
  ...
  if (process.env.DOMAIN_NAME) {
    console.log(`Frontend URL: ${process.env.DOMAIN_NAME}`);
  } else {
    console.log(`Frontend URL: http://localhost:3005`);
  }
